...
class Env:
    ...
    def market(self):
        ...
        # グリーンは油価の半分以下にならないように…
        self.p_green = max(self.p_green, self.p_oil/2) * (1 + random.uniform(-0.10, 0))
        self.pv_green = max(min(180, 180*200*4/(self.total_n_green+1)), 70) * (1 + random.uniform(-0.05, 0.05))

    def feebate(self):
        # フィーベイトのペナルティ・リベート適用
        sum_oil  = self.total_n_oil
        sum_green = self.total_n_green
        penalty_rate = (self.p_green - self.p_oil*self.feebate_rate) * sum_green/sum_oil if sum_oil else 0
        rebate_rate  = (self.p_green - self.p_oil*self.feebate_rate) if sum_oil else 0

        for a in self.agents:
            a.benefit -= max(0, penalty_rate * a.n_oil)
            a.benefit += max(0, rebate_rate  * a.n_green)
